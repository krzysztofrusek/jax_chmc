...
from diffrax import AbstractSolver,AbstractImplicitSolver
import jax
...
class Rattle(AbstractImplicitSolver):
    ...
    def step(
        ...
        def eq2(x: RattleVars, args=None):
            ...
            zero = (
                p0 - step_size * 0.5 * (vjp_fun(x.lam)[0] + dH_dq(x.p_1_2, q0)) - x.p_1_2,
                q0 - step_size * 0.5 * (dH_dp(x.p_1_2, q0) + dH_dp(x.p_1_2, x.q_1)) - x.q_1,
                self.constrain(x.q_1),
        ...
        def eq3(x: RattleVars, args=None):
            ...
            _, vjp_fun_mu = jax.vjp(self.constrain, x.q_1)
            zero = (
                p0 - control1_half_1*vjp_fun(x.lam)[0] - term_1.vf_prod(t0, q0, args, control1_half_1) - x.p_1_2,
                q0 + term_2.vf_prod(t0, x.p_1_2, args, control2_half_1) + term_2.vf_prod(midpoint, x.p_1_2, args, control2_half_2)  - x.q_1,
                self.constrain(x.q_1),
                x.p_1_2 - term_1.vf_prod(midpoint, x.q_1, args, control1_half_2) - control1_half_2 * vjp_fun_mu(x.mu)[0]  - x.p_1,
        ...
        def eq4(x: RattleVars, args=None):
            ...
            zero = (
                (p0**ω - control1_half_1*(vjp_fun(x.lam)[0])**ω + term_1.vf_prod(t0, q0, args, control1_half_1)**ω - x.p_1_2**ω).ω,
                (q0**ω + term_2.vf_prod(t0, x.p_1_2, args, control2_half_1)**ω + term_2.vf_prod(midpoint, x.p_1_2, args, control2_half_2)**ω  - x.q_1**ω).ω,
                self.constrain(x.q_1),
                (x.p_1_2**ω + term_1.vf_prod(midpoint, x.q_1, args, control1_half_2)**ω - (control1_half_2 * vjp_fun_mu(x.mu)[0]**ω)  - x.p_1**ω).ω,
                jax.jvp(self.constrain, (x.q_1,), (term_2.vf(t1,x.p_1,args),))[1]
            )
            return zero
        ...
        #solver = NewtonNonlinearSolver(rtol=1e-4, atol=1e-6)
        #eq2(init_vars)
